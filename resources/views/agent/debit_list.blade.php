...
<div class="main-container">
  ...
  <aside data-mcs-theme="minimal-dark" style="background-image: url{{asset('build/images/backgrounds/11.jpg')}}" class="main-sidebar closed mCustomScrollbar">
    <ul class="list-unstyled navigation mb-0">
      ...
      <li><a href="{{ url('/agent/debit_list') }}" class="active bubble"><i class="ti-blackboard"></i> Debts</a></li>
  ...
  <div class="page-container">
    <div class="page-header clearfix">
      <div class="pull-left">
              <a href="{{ url('agent/debit_list') }}" class="active mb-15 btn btn-raised btn-primary">Debt List</a>
              <a href="{{ url('agent/payment') }}" class="mb-15 btn btn-raised btn-primary">Payment</a>
            </div>
          </div>
          ...
          <div class="widget">
            ...
            <div class="widget-body">
              ...
              <table id="customer-list" style="width: 100%" class="table table-hover dt-responsive nowrap">
                ...
                <tbody>
                @foreach($assetProperties as $key => $assetProperty)
                  <tr>
                    <td class="text-center">
                      {{ $key + 1 }}
                    </td>
                    <td>{{ $assetProperty->designation }} {{ $assetProperty->firstname }} {{ $assetProperty->lastname }}</td>
                    <td>{{ $assetProperty->description }}</td>
                    <td>{{ $assetProperty->address }}</td>
                    <td>{{ $assetProperty->rent_dues_status }}</td>
                    <td>{{ $assetProperty->occupation_date }}</td>
                  </tr>
                  @endforeach
                </tbody>
              </table>
